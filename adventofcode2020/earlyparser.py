"""
Source: https://github.com/hardik-vala/earley-parser/blob/master/earleyparser.py
Earley Parser.
@author: Hardik
"""
    ...
    def __eq__(self, other):
        if type(other) is EarleyState:
            return (
                ...
                and self.sent_pos == other.sent_pos
            )
    ...
    def __str__(self):
        def str_helper(state):
            return (
                ...
                + state.rule.lhs
                + " -> "
                + " ".join(state.rule.rhs[: state.dot] + ["*"] + state.rule.rhs[state.dot :])
...
class Chart(object):
    ...
    def init(l):
        ...
        return Chart(
            [(ChartEntry([]) if i > 0 else ChartEntry([EarleyState.init()])) for i in range(l)]
        )
...
class EarleyParse(object):
    ...
    def predictor(self, state, pos):
        ...
        for rule in self.grammar[state.next()]:
            self.chart[pos].add(
                EarleyState(rule, dot=0, sent_pos=state.chart_pos, chart_pos=state.chart_pos)
            )
    ...
    def scanner(self, state, pos):
        ...
        if state.chart_pos < len(self.words):
            ...
            if any((word in r) for r in self.grammar[state.next()]):
                self.chart[pos + 1].add(
                    EarleyState(
                        Rule(state.next(), [word]),
                        dot=1,
    ...
    def has_parse(self):
        ...
        for state in self.chart[-1]:
            if (
                state.is_complete()
                and state.rule.lhs == "S"
                and state.sent_pos == 0
                and state.chart_pos == len(self.words)
            ):
                return True
